trigger CSUOEE_StopInvoiceCancellation on csuoee__Noncredit_Invoice_Payment__c (after insert, after update) {
    ...
    List<csuoee__Noncredit_Invoice__c> toUpdate = [select Id, csuoee__Cancel_At__c, csuoee__Is_Cancelled__c from csuoee__Noncredit_Invoice__c where Id in :invoiceIds and (csuoee__Is_Cancelled__c = true or csuoee__Cancel_in_Progress__c = true)];
    if(!toUpdate.isEmpty()) {
